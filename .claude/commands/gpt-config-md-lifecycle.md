---
name: gpt-config-md-lifecycle
description: Workflow command scaffold for gpt-config-md-lifecycle in chatgpt-for-data-analytics-bootcamp.
allowed_tools: ["Bash", "Read", "Write", "Grep", "Glob"]
---

# /gpt-config-md-lifecycle

Use this workflow when working on **gpt-config-md-lifecycle** in `chatgpt-for-data-analytics-bootcamp`.

## Goal

Creation, update, and deletion of GPT configuration markdown files for various analytics tasks.

## Common Files

- `day 1/gpt-configs/*.md`

## Suggested Sequence

1. Understand the current state and failure mode before editing.
2. Make the smallest coherent change that satisfies the workflow goal.
3. Run the most relevant verification for touched files.
4. Summarize what changed and what still needs review.

## Typical Commit Signals

- Create, update, or delete a markdown file in day 1/gpt-configs/
- Commit the change with an appropriate message (Create, Update, or Delete)

## Notes

- Treat this as a scaffold, not a hard-coded script.
- Update the command if the workflow evolves materially.